                    ...
                    <table class="w-auto" >
                        ...
                        <tbody>
                            @foreach($diagnosticos as $diagnostico)
                                <tr>
                                <td class="text-md px-4 py-2">{{ $diagnostico->cita->paciente->nombre_completo() }}</td>
                                <td class="text-md px-4 py-2">{{ $diagnostico->enfermedad->nombre }}</td>
                            
                                <td class="text-md px-4 py-2">{{ $diagnostico->cita->fecha }} {{ $diagnostico->cita->hora }}</td>
                              
                                <td>
                                    <a type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded-lg text-sm px-2 py-0.5 text-center mr-2 mb-2" href="{{route('EliminarDiagnostico',Crypt::encrypt($diagnostico->id))}}">Eliminar</a>
                                </td>
